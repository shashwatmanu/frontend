...
export const TaskManager = () => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const response = await axios.get(`${backendLink}/tasks`);
  ...
  const handleSave = async () => {
    ...
    try {
      if (isEditing) {
        await axios.patch(`${backendLink}/tasks/${taskData._id}`, {
        ...
        await axios.post(`${backendLink}/tasks`, formData);
      }
      const response = await axios.get(`${backendLink}/tasks`);
      setTasks(response.data);
      handleClose();
  ...
  const handleMarkAsDone = async (taskId) => {
    try {
      await axios.patch(`${backendLink}/tasks/${taskId}`, {
        status: "DONE",
      });
      const response = await axios.get(`${backendLink}/tasks`);
      setTasks(response.data);
  ...
  const handleDelete = async (taskId) => {
    if (window.confirm("Are you sure you want to delete this task?")) {
      try {
        await axios.delete(`${backendLink}/tasks/${taskId}`);
        const response = await axios.get(`${backendLink}/tasks`);
        setTasks(response.data);
      } catch (err) {
